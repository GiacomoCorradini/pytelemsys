...
from utils.conversion import darboux_to_cartesian
...
class TrackData:
    ...
    def plot_track_2D(self, ax:plt.figure, plot_kerbs:bool = False) -> None:
        ...
        plt.fill(np.concatenate([xL, xR[::-1]]), 
    ...
    def plot_track_3D(self, ax:plt.figure, plot_kerbs:bool = False) -> None:
        ...
        # Extract left and right margin points
        xL, yL, zL = self.track.x_margin_no_kerb_L, self.track.y_margin_no_kerb_L, self.track.z_margin_no_kerb_L
        xR, yR, zR = self.track.x_margin_no_kerb_R, self.track.y_margin_no_kerb_R, self.track.z_margin_no_kerb_R
        ...
        verts = [list(zip(np.concatenate([xL, xR[::-1]]),
                          np.concatenate([yL, yR[::-1]]),
    ...
    def _plot_kerbs_2D(self, ax:plt.figure, num_stripes:int = 1000) -> None:
        ...
        for i in range(num_stripes):
            ...
            # Left kerb
            plt.fill(np.concatenate([xL[idx_start:idx_end], xLk[idx_start:idx_end][::-1]]),
                     np.concatenate([yL[idx_start:idx_end], yLk[idx_start:idx_end][::-1]]),
                     color=color, alpha=0.8)

            # Right kerb
            plt.fill(np.concatenate([xR[idx_start:idx_end], xRk[idx_start:idx_end][::-1]]),
    ...
    def _plot_kerbs_3D(self, ax:plt.figure, num_stripes:int = 1000) -> None:
        ...
        xL, yL, zL = self.track.x_margin_no_kerb_L, self.track.y_margin_no_kerb_L, self.track.z_margin_no_kerb_L
        xR, yR, zR = self.track.x_margin_no_kerb_R, self.track.y_margin_no_kerb_R, self.track.z_margin_no_kerb_R
        xLk, yLk, zLk = self.track.x_margin_kerb_L, self.track.y_margin_kerb_L, self.track.z_margin_kerb_L
        xRk, yRk, zRk = self.track.x_margin_kerb_R, self.track.y_margin_kerb_R, self.track.z_margin_kerb_R
        ...
        for i in range(num_stripes):
            ...
            # Left kerb
            verts = [list(zip(np.concatenate([xL[idx_start:idx_end], xLk[idx_start:idx_end][::-1]]),
                              np.concatenate([yL[idx_start:idx_end], yLk[idx_start:idx_end][::-1]]),
                              np.concatenate([zL[idx_start:idx_end], zLk[idx_start:idx_end][::-1]])))]

            road_surface = Poly3DCollection(verts, color=color, alpha=0.8)
            ax.add_collection3d(road_surface)

            # Right kerb
            verts = [list(zip(np.concatenate([xR[idx_start:idx_end], xRk[idx_start:idx_end][::-1]]),
                              np.concatenate([yR[idx_start:idx_end], yRk[idx_start:idx_end][::-1]]),
                              np.concatenate([zR[idx_start:idx_end], zRk[idx_start:idx_end][::-1]])))]
